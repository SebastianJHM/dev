#Conjuntos
set A;#Proveedores Externos y ProveedoresInternos
set B;#Laboratorios
set C;#Posibles Centros de Distribución
set D;#Farmaceuticas, EPS y Hospitales
set E;#Todos los nodos de la red
set F;#Principios Activos y Excipientes
set G;#Medicamentos
set H;#Tipo de Centro de Distribución

#Parámetros
param P1{E,E};#Distancia entre los nodos de la red
param P2{A,F};#Oferta de materia prima de un proveedor(kg)
param P3{A,F};#Costo por kilo de materia prima($/kg)
param P4{B};#Costo de adquir el laboratorio($)
param P5{B,G};#Costo de producir medicamento($/kg)
param P6{B};#Capaidad de recepcion de materia prima por un laboratorio(Kg)
param P7{F,G,B};#Requerimiento Médico de materia prima para hacer un kg de medicamento en un laboratorio
param P8{C};#Si el centro puede ser tipo 1
param P9{C};#Si el centro puede ser tipo 2
param P10{C,H};#Costo de apertura de un centro de distribución según tipo
param P11{C};#Costo de No abriruncentrodedistribución
param P12{C,G,H};#Costo de empaquetar en un centro de distribución cierto de medicamento según el tipo
param P13{H};#Capacidad de los centros de distribución según tipo
param P14{D,G};#Demanda de un cliente de un  tipo de medicamento

#Variables
var x{C,H},binary;#1 Si abro un centro de distribución de cierto tipo.0 dlc
var y{C},binary;#1 Si abro un centro de distribución
var z{B},binary;#1 Si abro un laboratorio.0 dlc
var pl1{A,B},binary;#1 Si envío de un proveedor.0 dlc
var pl2{A,B,F},>=0;#Cantidad enviada de un proveedor a un laboratorio de materia prima
var lc1{B,C},binary;#1 Si envío de un laboratorio a un centro de distribución
var lc2{B,C,G},>=0;#Cantidad enviada de un centro de distribución a un laboratorio
var lc3{B,G},>=0;#Producción de un laboratorio de cierto mediamento
var lc4{B,F},>=0;#Cantidad recibida por un laboratorio de materia prima
var cc1{C,D,H},binary;#1 Sienvío de un centro de distribución de cierto  tipo a un cliente.0 dlc.
var cc2{C,D},binary;#1Si envío de un centro de distribución a un cliente.0 dlc.
var cc3{C,D,G,H},>=0;#Cantidad de medicamento enviada de un centro de distribucion de cierto tipo a un cliente
var cc4{C,D,G},>=0;#Cantidad de medicamento enviada de un centro de distribucion a un cliente
var cc5{C,G,H};#Cantidad enviada de un centro de distribución de un  tipo de medicamento de cierto tipo
var c1,>=0;#Costo total de compra de materia prima
var c2,>=0;#Costo total de adquirir laboratorios
var c3,>=0;#Costo total de producción en los laboratorios
var c4,>=0;#Costo total de apertura de centros de distribución
var c5,>=0;#Costo total de no apertura de centros de distribución
var c6,>=0;#Costo total de empaquetamiento en un centro de distribución
var c7,>=0;#Costo total de transporte
var c8,>=0;#Costo total de viaje
var v1{A,B}, integer, >=0;#Número de viaje entre un proveedor y un laboratorio
var v2{B,C}, integer, >=0;#Número de viaje entre un laboratorio y un centro de distribución
var v3{C,D}, integer, >=0;#Número de viaje entre un centro de distribución a un cliente
...
subj to res6{a in A, b in B, f in F}: pl2[a,b,f] <= 100000*pl1[a,b];
subj to res0{a in A, b in B}: pl1[a,b] <= sum{f in F}pl2[a,b,f];
subj to res7{a in A, f in F}: sum{b in B} pl2[a,b,f] <= P2[a,f];
subj to res8{a in A}: sum{b in B}pl1[a,b] <= 3;
subj to res9{b in B}: sum{a in A,f in F} pl2[a,b,f] <= P6[b];
subj to res10{b in B, f in F}: lc4[b,f] = sum{a in A} pl2[a,b,f];
subj to res11{b in B, f in F}: lc4[b,f] = sum{g in G} P7[f,g,b] * lc3[b,g];
subj to res12{b in B, c in C}: lc1[b,c] <= z[b];
subj to res13{b in B, c in C}: lc1[b,c] <= y[c];
subj to res14{b in B, c in C, g in G}: lc2[b,c,g] <= 100000*lc1[b,c];
subj to res15{b in B, c in C}: lc1[b,c] <= sum{g in G}lc2[b,c,g];
subj to res16{b in B, g in G}: sum{c in C}lc2[b,c,g] = lc3[b,g];
subj to res17{c in C, d in D,h in H}: cc1[c,d,h] <= x[c,h];
subj to res18{c in C, d in D, g in G,h in H}: cc3[c,d,g,h] <= 100000*cc1[c,d,h];
subj to res19{c in C, d in D}: cc2[c,d] = sum{h in H}cc1[c,d,h];
subj to res20{c in C, d in D,g in G}: cc4[c,d,g] = sum{h in H}cc3[c,d,g,h];
subj to res21{c in C, h in H}: sum{d in D,g in G}cc3[c,d,g,h] <= P13[h];
subj to res22{d in D}: sum{c in C}cc2[c,d] = 1;
subj to res23{d in D, g in G}: sum{c in C}cc4[c,d,g] >= P14[d,g];
subj to res24{c in C,g in G}: sum{d in D}cc4[c,d,g] = sum{b in B}lc2[b,c,g];
subj to res25{c in C,g in G,h in H}: cc5[c,g,h] = sum{d in D}cc3[c,d,g,h];
...
printf: "------------------------\n";
printf: "\nCOSTOS:\n";
printf: "Costos de compra materia prima laboratorios: "&c1&"\n";
printf: "Costos de adquirir laboratorios: "&c2&"\n";
printf: "Costos de producción laboratorios: "&c3&"\n";
printf: "Costos de apertura centros de distribución: "&c4&"\n";
printf: "Costos de no apertura centros de distribución: "&c5&"\n";
printf: "Costos de empaquetamiento centros de distribución: "&c6&"\n";
printf: "Costos total de transporte: "&c7&"\n";
printf: "Costos total de viajes: "&c8&"\n";

printf: "\n------------------------\n";
printf: "\nCENTROS DE DISTRIBUCIÓN A ABRIR:\n";
for{h in H}{
	for{c in C:x[c,h]=1}{
		printf: "Se abre centro de distribución en "&c&" de tipo "&h&"\n";
	}
}
printf: "\nLABORATORIOS A ABRIR:\n";
for{b in B:z[b]=1}{
	printf: "Se abre laboratorio en "&b&"\n";
}
printf: "\n------------------------\n";
...
for{b in B:z[b]=1}{
	printf: "Laboratorio "&b&": ";
	printf: "\nRecepción: ";
	for{f in F}{
		printf: f&": "&lc4[b,f]&" kilos ... ";
	}
	printf: "\nFabricación: ";
	for{g in G}{
		printf: g&": "&lc3[b,g]&" kilos ... ";
	}
	printf: "\n";
	printf: "\n";
}
