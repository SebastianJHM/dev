...
def obtener_resultados(instance):
    ...
    for p in instance.P:
        ...
        #rof
    #rof
    return(secuencia, round(pyo.value(instance.Tardanza_Total)), round(pyo.value(instance.Makespan)) )
#fed

def modelo_tardanza( TP, dd, tl ):
    ...
    secuencia, makespan, tardanza = obtener_resultados(instance)
    return(secuencia, makespan, tardanza)
    
    # if (results.solver.status == pyo.SolverStatus.ok and results.solver.termination_condition == pyo.TerminationCondition.feasible):
